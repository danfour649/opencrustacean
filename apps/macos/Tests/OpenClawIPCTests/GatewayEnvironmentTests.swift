...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        #expect(Semver.parse("OpenCrustacean 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  OpenCrustacean 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("OpenCrustacean 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("OpenCrustacean 2026.4.2-1 (d74a122)")
