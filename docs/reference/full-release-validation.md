...
| Windows node candidate E2E | **Job:** `windows_node_release_candidate_e2e`<br />**Backing workflow:** `openclaw/openclaw-windows-node` release-candidate E2E<br />**Tests:** downloads the same immutable candidate tarball, verifies its SHA-256 and version, installs it into a fresh WSL gateway, then proves tray connection, Windows-node capability registration, MCP, and MXC execution.<br />**Runs:** every full release validation (`rerun_group=all`). |
